...
Begin VB.Form rpt_offboard 
   ...
   End
End
Attribute VB_Name = "rpt_offboard"
...
main.lbltitle.Caption = "Personnel OffBoard List"
 
Me.Top = 10
Me.Left = 10
 dtp_from.Value = Format(Date, "dd/mm/yyyy H:MM:ss")
 dtp_to.Value = Format(Date, "dd/mm/yyyy H:MM:ss")
...
Dim fso As New FileSystemObject
   ...
   fs.WriteLine "    <table border=1 cellspacing=0 BORDERCOLOR=gray width=95%>"
                ...
                fs.WriteLine "           <td colspan=6 ><b>PERSONNEL OffBoard LIST Between " & Format(dtp_from.Value, "dd/MMM/yy") & "  -  " & Format(dtp_to.Value, "dd/MMM/yy") & "</td>"
...
'   fs.WriteLine "            <td Nowrap><font color=white>NIC</td>"
   ...
   fs.WriteLine "            <td ><font color=white>Date OffBoard</td>"
   fs.WriteLine "            <td ><font color=white>Days OnBoard</td>"
   fs.WriteLine "            <td ><font color=white>Reason OffBoard</td>"
...
rs.Open "select * from employee e, onboard o , offboard ofb where e.emp_no = o.ob_empno  and e.emp_no=ofb.empno and ofb.dateoffboard between '" & Format(dtp_from.Value, "mm/dd/yyyy") & "' and '" & Format(dtp_to.Value, "mm/dd/yyyy") & "'  and o.location='" & cbo_location.Text & "'  order by e.emp_classification", Cn, 3, 2
While Not rs.EOF
diff = 0
fs.WriteLine "        <tr bgcolor=white height=15 class=TableFont>"
...
fs.WriteLine "            <td  >&nbsp;</td>"
Else
fs.WriteLine "            <td  Nowrap>" & Format(rs!ob_dateonboard, "dd/MM/yyyy H:mm:ss") & "</td>"
End If
If rs!dateoffboard = "" Then
fs.WriteLine "            <td  >&nbsp;</td>"
Else
fs.WriteLine "            <td  Nowrap>" & Format(rs!dateoffboard, "dd/MM/yyyy H:mm:ss") & "</td>"
End If
diff = DateDiff("d", rs!ob_dateonboard, rs!dateoffboard)
 
fs.WriteLine "            <td  Nowrap>" & diff & "</td>"
If rs!reason = "" Then
fs.WriteLine "            <td  >&nbsp;</td>"
Else
fs.WriteLine "            <td  Nowrap>" & rs!reason & "</td>"
End If
If rs!ob_shift = "" Then
fs.WriteLine "            <td  >&nbsp;</td>"
Else
